...
class TestDataCleaner(unittest.TestCase):
    ...
    def test_drop_invalid_rows_removes_rows_with_missing_values(self):
        ...
        # 1. Verificar que no hay valores faltantes en las columnas especificadas
        self.assertEqual(result["name"].isna().sum(), 0)
        self.assertEqual(result["age"].isna().sum(), 0)

        # 2. Verificar que el DataFrame resultante tiene menos filas que el original
    ...
    def test_trim_strings_strips_whitespace_without_changing_other_columns(self):
        ...
        # Copia para verificar no mutación
        df_original = df.copy()

        result = cleaner.trim_strings(df, ["name"])

        # 1. Verificar que el DataFrame original NO fue modificado
        self.assertEqual(df.loc[0, "name"], df_original.loc[0, "name"])
        self.assertEqual(df.loc[3, "name"], df_original.loc[3, "name"])

        # 2. Verificar que el DataFrame resultante SÍ tiene los strings limpiados
        self.assertEqual(result.loc[0, "name"], "Alice")
        self.assertEqual(result.loc[1, "name"], "Bob")
        self.assertEqual(result.loc[3, "name"], "Carol")

        # 3. Verificar que las columnas no especificadas permanecen iguales
    ...
    def test_remove_outliers_iqr_removes_extreme_values(self):
        ...
        # El valor extremo 120 NO debe estar
        self.assertNotIn(120, result["age"].values)

        # Al menos un valor no extremo debe permanecer
